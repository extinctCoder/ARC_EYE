...
def get_direction(temp_x_axis, temp_y_axis, temp_small_radious, frame):
    if temp_x_axis > 0 and temp_x_axis <= 200:
        text = "Right Turn"
        data = "03,60"
        print ("Right Turn          >>>>>")

     if temp_x_axis >= 440 and temp_x_axis < 640:
        text = "Left Turn"
        data = "02,60"
        print ('Left Turn           <<<<<')

     if temp_x_axis >= 200 and temp_x_axis <= 440:
    ...
    #'''if temp_x_axis > 420 and temp_x_axis <= 640:
        '''text = "Right Turn"
        data = "03,60"
        print ('Right Turn          >>>>>')

        # byte_data = bytes("04,30", 'utf-8')
    if temp_x_axis >= 0 and temp_x_axis < 220:
        text = "Left Turn"
        data = "02,60"
        print ('Left Turn           <<<<<')

        # byte_data = bytes("02,20", 'utf-8')
    if temp_x_axis >= 220 and temp_x_axis <= 420:
        text = "Forward Movement"
        data = "00,60"
        print ('Forward Movement    ^^^^^')
                ...
                for(imageI, imageC)in enumerate(contours):
                    ...
                    if imageRadius > 10:
                        ...
                        '''cv2.putText(frame, "[" + str(imageX) + "," + str(imageY) + "]", (int(
                            imageX), int(imageY)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)'''
